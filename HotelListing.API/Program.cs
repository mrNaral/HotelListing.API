...
builder.Services.AddSwaggerGen( options => {
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "Hotel Linsting API", Version = "v1" });
    options.AddSecurityDefinition(JwtBearerDefaults.AuthenticationScheme, new OpenApiSecurityScheme
    {
        Description = @"JWT Authorization header using Bearer scheme.
                      Enter 'Bearer' [SPACE] and then your token in the text input below.
                      Example: 'Bearer 123sdfjh132...'",
        Name = "Authorization",
...
{
    Predicate = heathcheck => heathcheck.Tags.Contains("custom"),
    ResultStatusCodes =
    {
...
{
    Predicate = heathcheck => heathcheck.Tags.Contains("database"),
    ResultStatusCodes =
    {
        [HealthStatus.Healthy] = StatusCodes.Status200OK,
        [HealthStatus.Unhealthy] = StatusCodes.Status503ServiceUnavailable,
...
static Task WriteResponse(HttpContext httpContext, HealthReport healthReport)
{
...
class CustomHealthCheck : IHealthCheck
{
    public Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        //Custom checks Logic etc etc etc...
        if (isHealthy)
        {
